        ...
        value: function table() {
            var initDataTable = $('#myTable').DataTable({
                ...
                columnDefs: [                   
                    {
                        ...
                        render: function render(data, type, row, meta) {
                            if (row.ParentID == "0") {
                                return "<a onclick='OpenAccordian(this," + row.ID + ")' class='accordian lnk" + row.ID  +"'>+</a><a href=\"category-modify.aspx?id=" + row.ID + "\" class=\"mr-1\"><b>" + row.Name + "</b></a>";                            
                            }
                            else {
                                if (row.TreeLevelNo == 2) {
                                    return "<a onclick='OpenAccordian(this," + row.ID + ")' class='accordian lnk" + row.ID +"'>+</a><a href=\"category-modify.aspx?id=" + row.ID + "\" class=\"mr-1\">" + row.Name + "</a>";
                                }
                                else {
                                    return "<a href=\"category-modify.aspx?id=" + row.ID + "\" class=\"mr-1\">" + row.Name + "</a>";
                                }
                            }
                    ...
                    },
                    {
                        ...
                        render: function render(data, type, row, meta) {
                            //TreeLevelNo  
                            if (row.TreeLevelNo == "3") {
                                return "<a href='question-list.aspx?cid=" + row.ID + "' class='font17 qcount'>" + row.Questions +"</a>";
                            }
                            else {
                                return "<span class='spnqcount'></span>";
                            }
                    ...
                    },
                    {
                        targets: 4,
                        render: function render(data, type, row, meta) {                            
                            return "<a data-toggle='tooltip' data-placement='bottom' title='Edit' class='btn btn-sm btn-icon btn-secondary' href='category-modify.aspx?id=" + row.ID + "'><i class=\"fa fa-pencil-alt\"></i></a>";                                
                        }
                    },
                    {
                        targets: 5,
                ...
                createdRow: function (row, data, dataIndex) {
                    ...
                    if (data.TreeLevelNo == 1) {
                        $(row).addClass('MainParent mainid' + data.ID);
                        $(row).attr('rel',data.ID);
                    }
                    else if (data.TreeLevelNo == 2) {
                        $(row).addClass('pad10 hide cls' + data.ParentID + ' mainid' + data.ID);
                        $(row).attr('rel', data.ID);
                    }
                    else if(data.TreeLevelNo == 3) {
                        $(row).addClass('pad20 hide cls' + data.ParentID + ' mainid' + data.ID);
                        $(row).attr('rel', data.ID);
                    }
                }
            });
